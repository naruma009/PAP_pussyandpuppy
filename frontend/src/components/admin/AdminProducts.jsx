...
export default function AdminProducts() {
  ...
  return <section className="admin-products container"><div className="section-heading"><div><span className="eyebrow">PAP Admin</span><h1>{t("manageProducts")}</h1><p>{t("manageProductsIntro")}</p></div></div>{success && <p className="admin-success" role="status">{success}</p>}{syncWarning && <div className="admin-sync-warning" role="alert"><span>{t("catalogSyncFailed")}: {syncWarning}</span><button type="button" onClick={retrySync}>{t("retry")}</button></div>}<ProductAdminForm editingProduct={editingProduct} busy={busy} error={error} onCancel={() => { setEditingProduct(null); setError(""); }} onSubmit={submit} /><AdminProductTable products={products} busy={busy} onEdit={edit} onDelete={remove} /></section>;
}
