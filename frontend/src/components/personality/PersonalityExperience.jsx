...
export default function PersonalityExperience() {
  const { pathname } = useLocation(); const { language, petMode, playSound } = usePreferences(); const refs = useRef([]), controllersRef = useRef([]); const [menuKind, setMenuKind] = useState(null); const [controllerList, setControllerList] = useState([]);
  ...
  useEffect(() => {
    ...
    controllersRef.current = controllers; setControllerList(controllers);
    if (reducedMotion) return () => { controllersRef.current = []; setControllerList([]); controllers.forEach((controller) => controller.destroy()); };
    ...
    const destroy = () => { if (destroyed) return; destroyed = true; controllersRef.current = []; setControllerList([]); removeEventListener("pointermove", onPointerMove); removeEventListener("scroll", onScroll); removeEventListener("pagehide", destroy); if (pointerFrame) cancelAnimationFrame(pointerFrame); if (scrollFrame) cancelAnimationFrame(scrollFrame); observer.disconnect(); controllers.forEach((controller) => controller.destroy()); };
  ...
  return <><div className="pap-mascot-stage" data-testid="mascot-stage">{kinds.map((kind, index) => <Mascot key={kind} kind={kind} language={language} onActivate={activate} ref={(node) => { refs.current[index] = node; }} />)}</div><PetInteractions key={`actions-${pathname}-${petMode}`} getControllers={getControllers} menuKind={menuKind} onCloseMenu={closeMenu} /><PetChat key={`chat-${pathname}-${petMode}`} getControllers={getControllers} mode={petMode} /><ChaosExperience controllers={controllerList} mode={petMode} reducedMotion={mediaMatches("(prefers-reduced-motion: reduce)")} playSound={playSound} /></>;
}
