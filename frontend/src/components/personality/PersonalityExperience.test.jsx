...
describe("PersonalityExperience", () => {
  it.each([["cat", ["cat"]], ["dog", ["dog"]], ["both", ["cat", "dog"]]])("renders %s mode species", (mode, kinds) => {
    ...
    expect(screen.getAllByRole("img").map((node) => node.dataset.kind)).toEqual(kinds);
  });
  it("renders the species selected by Pet Mode with localized accessible names", () => {
    ...
    expect(screen.getByLabelText("PAP cat")).toBeInTheDocument(); expect(screen.getByLabelText("PAP dog")).toBeInTheDocument();
  });
  ...
  it("keeps Thai labels and dark theme compatibility", () => { localStorage.setItem("pap-theme", "dark"); view(); expect(screen.getByLabelText("แมว PAP")).toBeInTheDocument(); expect(document.documentElement).toHaveAttribute("data-theme", "dark"); });
});
