...
import MoodAssessment from "../components/personality/MoodAssessment";
...
export default function HomePage() {
  ...
  return <>
    <section className="hero"><div className="container hero-grid"><div><span className="eyebrow">Happy pets, happy home</span><h1>{t("homeTitle")}<br /><span>{t("homeTitleAccent")}</span></h1><p>{t("homeIntro")}</p><div className="hero-actions">{status === "success" ? <MoodAssessment products={products} /> : <span className="mood-cta-slot" aria-hidden="true" />}<Link className="button secondary" to="/products?reset=1#product-grid">{t("viewAllProducts")}</Link></div></div><div className="hero-art" aria-label={t("happyPets")}><div className="blob" /><span className="spark one">✨</span><span className="spark two">★</span><span className="spark three">✦</span><div className="pet-stage"><span className="pet-face">🐱</span><span className="pet-face">🐶</span></div></div></div></section>
