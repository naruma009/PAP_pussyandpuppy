...
export default function CheckoutGuard({ children, completed = false }) {
  ...
  useEffect(() => {
    if (completed || customerStatus !== "ready" || !cartReady) return;
  ...
  }, [completed, customer, customerStatus, cart, cartReady, navigate]);
  if (completed) return children;
  if (catalogStatus === "error" && !cartReady) return <section className="foundation-placeholder container"><CatalogError onRetry={retryCatalog} /></section>;
