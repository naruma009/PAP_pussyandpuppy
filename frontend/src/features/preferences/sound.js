...
export function playCommerceSound(enabled, type = "click", species = "both") {
  ...
  const notes = type === "success" ? [523, 659, 784] : type === "cart" ? [420, 620] : type === "mood" ? [392, 523, 659] : type === "chaos" ? [220, 330, 440] : type === "feed" ? (species === "cat" ? [680, 820] : [360, 520]) : type === "pet" ? (species === "cat" ? [420, 500] : [520, 660]) : [480];
  notes.forEach((frequency, index) => {
    const oscillator = audioContext.createOscillator();
    const gain = audioContext.createGain();
    const start = audioContext.currentTime + index * 0.055;
    oscillator.type = type === "chaos" ? "triangle" : "sine";
