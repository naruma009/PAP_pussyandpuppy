document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderSummary() {
    ...
    itemsRoot.innerHTML = cart.map((entry) => { const product = products.find((item) => item.id === entry.id); return product ? `<div class="checkout-item"><span>${safe(product.name)} × ${entry.qty}</span><strong>${money(product.price * entry.qty)}</strong></div>` : ""; }).join("");
  ...
  function renderAddress() { const customer = customerData(); const hasAddress = customer.address || customer.province; addressPreview.innerHTML = `<strong>Shipping Address</strong><p>${hasAddress ? `${safe(customer.fullName)}<br>${safe(customer.address)} ${safe(customer.district)}<br>${safe(customer.province)} ${safe(customer.postalCode)}<br>${safe(customer.phone)}` : "กรอกข้อมูลเพื่อดูที่อยู่จัดส่ง"}</p>`; }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    document.querySelector("#checkout-main").innerHTML = `<section class="order-confirmation"><div class="big-emoji">✓</div><span class="eyebrow">Order confirmed</span><h1>Thank you!</h1><p>Order <strong>${safe(result.order.id)}</strong> has been placed.</p><p>Total ${money(result.order.total)}</p><a class="button" href="home.html">Back to Home</a></section>`;
  });
